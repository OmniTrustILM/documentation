...
The build fetches 30+ OpenAPI specs from `https://api.otilm.com/` and renders them via Redocusaurus — it is slow and network-dependent.
...
- **`remarkSimplePlantumlPlugin.mjs`** — Converts fenced code blocks with language `plantuml` into rendered SVG diagrams via the PlantUML web service.
