...
The build downloads 53 OpenAPI documents from `https://api.otilm.com/` into `static/api-specs/`, so it
...
the switch to Scalar the cost is close to that baseline — measured with 52 documents it peaked at ~3.3 GB,
the bundling and generation phases took under 30 s, and it succeeded with a heap limit as low as 3072 MB.
...
53 OpenAPI documents are published at `/api/<id>` and rendered by **Scalar**. Everything is driven from
...
rather than a build error. `yarn verify-api-build` is the guard: it checks that every one of the 53
