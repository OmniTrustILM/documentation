...
When `Vault Profile` is a source profile, acces to a secret is based on access to the `Vault Profile`. When a source `Vault Profile` is disabled, operations on secrets in that vault are not allowed.
For managment of Vault Profiles, see [Vault Profile Management API](vault-profile-management.md).
