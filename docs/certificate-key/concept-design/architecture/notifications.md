...
- **Owner**: The notification is delivered to the owner of the specific object that triggered it (for example, the owner of a certificate). The actual user is resolved per object, based on the ownership association recorded on it.
- **Object**: The notification is sent to a contact stored on the triggering object itself (for example, an e-mail address held in a custom attribute on a certificate), rather than to a platform user, group, or role. Used for external delivery only. See [Resolving the Object recipient](#resolving-the-object-recipient) for how the contact details are determined.
...
If a required attribute is missing on the object, the recipient cannot be resolved and the notification is not delivered. Where the provider supports it, a single custom attribute may hold multiple values — for example, an e-mail provider notifies every address held in the mapped attribute.
