...
- **Object**: The recipient is resolved per object from the [custom attributes](../../settings/custom-attributes.md) of the object carried in the notification message. This type is used to notify an address stored on the object itself rather than a platform user, and applies to external delivery only. The notification instance's [mapping attributes](../core-components/notification.md#mapping-attributes), defined by the instance's connector, determine which custom attribute supplies each piece of recipient data required by the provider (for example the e-mail address). The object must carry the mapped custom attribute, and its content type must match the mapping attribute's content type. If a required mapped attribute is missing, the recipient cannot be resolved. For an e-mail provider, a single mapped attribute may hold multiple e-mail addresses, in which case all of them are notified.
