...
The [`AttributeCallback`](https://github.com/CZERTAINLY/CZERTAINLY-Interfaces/blob/master/src/main/java/com/czertainly/api/model/common/attribute/common/callback/AttributeCallback.java) contains the following properties:
...
| `mappings`        | set of [`AttributeCallbackMapping`](https://github.com/CZERTAINLY/CZERTAINLY-Interfaces/blob/master/src/main/java/com/czertainly/api/model/common/attribute/common/callback/AttributeCallbackMapping.java) | Mappings for the callback method, which defines how to use the data in context of the request path variables, query parameter, or body payload. | <span class="badge badge--success">Yes</span> |
...
:::info
Mappings have various options how to include the data from other `Attributes` and request additional action based on them. See the available options in [AttributeCallbackMapping](https://github.com/CZERTAINLY/CZERTAINLY-Interfaces/blob/master/src/main/java/com/czertainly/api/model/common/attribute/common/callback/AttributeCallbackMapping.java).
:::
...
```
This callback is used to retrieve resource objects for attributes with `Resource Object` content type. The context and method do not have to be specified, since this callback is called based on the content type.
In this case, mappings serve to filter resource objects, which will be retrieved from Core database, identified by their name and UUID. 
Formatting of a mapping is as follows:
 - "to" - is of format `{fieldIdentifier}.{operator}`, the values of the identifier and operator for a specific resource can be retrieved by calling API endpoint which lists the searchable information (`/v1/{resource}/search`). 
 The fieldIdentifier must be of field source `PROPERTY`.
 - "targets" - must contain "filter"
 - "value" - value to filter by

 If the mapping contains field "from", then the value of filter can be retrieved from other attribute and field "value" can be ommited.
...
The following diagram represents the callbacks model. Details can be found in the [CZERTAINLY Interfaces repository](https://github.com/CZERTAINLY/CZERTAINLY-Interfaces/tree/master/src/main/java/com/czertainly/api/model/common/attribute/common/callback).
